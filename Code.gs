...
function bootstrap(ss, weekIds) {
  ...
  var perfil = null;
  try {
    var pv = sheetGetJSON(ss, '_AppData', 'profile');
    perfil = pv ? JSON.parse(pv) : null;
  } catch(e) {}
  result.profile = perfil;

  // 2) IDs dels fulls compartits
  result.grupsSheetId  = sheetGetJSON(ss, '_AppData', 'grups_sheet_id')  || '';
  result.desdobSheetId = sheetGetJSON(ss, '_AppData', 'desdob_sheet_id') || '';
  ...
  var appDataBundle = loadAppData(ss, weekIds);
