...
const GRUP_HEADERS = [
  ...
  'Observació important','Gènere','PI','AM','Aspectes específics'
];
...
function getGrupsSpreadsheet(ss) {
  var id = sheetGetJSON(ss, '_AppData', 'grups_sheet_id');
  if (!id) return null;
...
function getGrupsSheetId(ss) {
  return { ok:true, id: sheetGetJSON(ss, '_AppData', 'grups_sheet_id') || '' };
}
const MATERIA_NOM = {
...
function handleRequest(e) {
  try {
    ...
    switch (action) {
      ...
      case 'updateNota':           result = updateNota(ss, body.materia, body.trimestre, body.itemId, body.studentId, body.punts, body.grup); break;
      case 'setNoEntregat':        result = setNoEntregat(ss, body.materia, body.trimestre, body.itemId, body.studentId, body.valor, body.grup); break;
      ...
      case 'getGrupAlumnes':         result = getGrupAlumnes(getGrupsSpreadsheet(ss) || ss, body.grup || p.grup); break;
      case 'saveGrupPersonal':       result = saveGrupPersonal(getGrupsSpreadsheet(ss) || ss, body.grup, body.rowId, body.dades); break;
      ...
      case 'getDesdoblament':        result = getDesdoblament(ss, body.curs || p.curs, body.linia || p.linia, body.assignatura || p.assignatura); break;
      case 'getGrupObs':             result = getGrupObs(ss, body.grup || p.grup); break;
...
function getGrupAlumnes(ss, grup) {
  ...
  if (!sh) return { ok:true, alumnes:[], grup:grup, existeix:false };
...
function getDesdobSpreadsheet(ss) {
  var id = sheetGetJSON(ss, '_AppData', 'desdob_sheet_id');
  if (!id) return null;
...
function getDesdobSheetId(ss) {
  return { ok:true, id: sheetGetJSON(ss, '_AppData', 'desdob_sheet_id') || '' };
}
...
/* ============================================================
   ...
   ============================================================ */
function updateNota(ss, materia, trimestre, itemId, studentId, punts, grup) {
  var nomBase=MATERIA_NOM[materia]; if(!nomBase)return{ok:false,error:'Materia desconeguda'};
...
/* ============================================================
   NO ENTREGAT
   ============================================================ */
function setNoEntregat(ss, materia, trimestre, itemId, studentId, valor, grup) {
  var nomBase=MATERIA_NOM[materia]; if(!nomBase)return{ok:false,error:'Materia desconeguda'};
  var sh=ss.getSheetByName(_notesTabName(trimestre, nomBase, grup)); if(!sh)return{ok:false,error:'Pestanya no trobada'};
  var lc=sh.getLastColumn();
  var metas=sh.getRange(1,1,1,lc).getNotes()[0];
