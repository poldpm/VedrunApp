...
async function openNotes(materia, trimestre, grup) {
  ...
  // Si l'assignatura és d'un grup concret, carrega'n els alumnes (per avaluar-los).
  // Si és el grup de tutoria, ja estan carregats.
  if (notesContext.grup && typeof _ensureGrupStudents === 'function') {
    await _ensureGrupStudents(notesContext.grup, materia);
  }
...
async function updateNota(itemId, studentId, punts) {
  ...
  _saveQueue = _saveQueue.then(async () => {
    ...
    try {
      const r = await appsScriptPost({ action:'updateNota', materia:notesContext.materia, trimestre:notesContext.trimestre, grup:notesContext.grup, itemId:p.itemId, studentId:p.studentId, punts:p.punts });
...
async function toggleNoEntregat(item, studentId, inp, chip) {
  ...
  if (config.scriptUrl) {
    try { await appsScriptPost({ action:'setNoEntregat', materia:notesContext.materia, trimestre:notesContext.trimestre, grup:notesContext.grup, itemId:item.id, studentId, valor:isNE }); }
