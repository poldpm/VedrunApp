...
/* --- Cache local per assignatura+trimestre (TTL 10 min) --- */
const _cache   = {};
const CACHE_MS = 10 * 60 * 1000;
function _cacheKey()      { return notesContext.materia + '_' + notesContext.trimestre; }
function _cacheGet()      { const c = _cache[_cacheKey()]; if (!c || Date.now() - c.ts > CACHE_MS) { delete _cache[_cacheKey()]; return null; } return c; }
function _cacheSet(d)     { _cache[_cacheKey()] = { ...d, ts: Date.now() }; }
function _cacheDel()      { delete _cache[_cacheKey()]; }
