...
async function _loadTutoriaGrup() {
  ...
  try {
    ...
    if (r.ok && r.alumnes && r.alumnes.length) {
      _tutoriaAlumnes = r.alumnes;
      // Substitueix la llista global d'alumnes per la del grup de tutoria
      students = r.alumnes.map(function(a) { return { id: a.id, nom: a.nom, genere: a.genere }; });
      // Omple les dades personals
      personal = {};
      r.alumnes.forEach(function(a) {
        personal[a.id] = {
          mare: a.mare, pare: a.pare, emailMare: a.emailMare, emailPare: a.emailPare,
          obs: a.obs, pi: a.pi, am: a.am, especific: a.especific,
          dataNaix: a.dataNaix, rowId: a.rowId,
        };
      });
      _saveMainToCache();
      _paintAllViews();
      // Recalcula els aniversaris al planning
...
async function _ensureGrupStudents(grup, materia) {
  ...
  if (_grupStudentsCarregat === clau) return; // ja carregat

  // Si és el grup de tutoria i no hi ha desdoblament, ja tenim els alumnes
  const esTutoria = (typeof _perfilTutorGrupKey === 'function') && _perfilTutorGrupKey() === grup;
    ...
    if (alumnes.length) {
      ...
      alumnes.forEach(a => {
        personal[a.id] = { mare:a.mare, pare:a.pare, emailMare:a.emailMare, emailPare:a.emailPare,
          obs:a.obs, pi:a.pi, am:a.am, especific:a.especific, dataNaix:a.dataNaix, rowId:a.rowId };
      });
