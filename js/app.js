...
async function loadAll() {
  ...
  try {
    ...
    // Carrega els events de Google Calendar del mes actual i el següent en segon pla
    // (perquè surtin al planning i a la pàgina d'inici sense obrir el calendari,
    //  cobrint també setmanes que creuen el canvi de mes)
    const _now = new Date();
    const _y = _now.getFullYear(), _m = _now.getMonth();
    const _nextY = _m === 11 ? _y + 1 : _y;
    const _nextM = _m === 11 ? 0 : _m + 1;
    Promise.all([
      _loadGCalEvents(_y, _m),
      _loadGCalEvents(_nextY, _nextM),
    ]).then(() => {
