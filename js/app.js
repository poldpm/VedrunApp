...
function showPage(pageId) {
  ...
  if (pageId === 'assoliments')  { if (_recentFullLoad()) renderAssoliments(); else _assimLoadFromSheets(_assimMateria, _assimTrim).then(() => renderAssoliments()); }
  if (pageId === 'comentaris')   { initComentaris(); renderComentRubrica(); }
  if (pageId === 'grups')        initGrups();
  if (pageId === 'calendari')    { if (_recentFullLoad()) renderCalendari(); else _calLoadFromSheets(new Date().getFullYear()).then(() => renderCalendari()); }
...
async function loadAll() {
  ...
  try {
    ...
    if (alumnesR.ok)  { students = alumnesR.alumnes; updateHomeCounters(); renderHome(); initComentaris(); }
    ...
    // Carrega totes les dades locals des del Sheets (per sincronitzar entre dispositius)
    await loadAllFromSheets();
    updateSync('ok', 'Sincronitzat'); updateStatSync();
    document.getElementById('setupBanner').style.display = 'none';
    // Refresca vistes obertes (ara amb les dades del Sheets ja carregades)
    const isVisible = id => !document.getElementById('page-' + id).classList.contains('page-hidden');
    ...
    // Precarrega el resum de notes en segon pla → obrir fitxes serà instantani
    if (typeof _prefetchNotesResum === 'function') _prefetchNotesResum();
    // Carrega Google Tasks en segon pla → la bombolla surt sense entrar a la pàgina
    loadGoogleTasksSilent();
    // Inicialitza el sistema de notificacions
    initNotifications();
  } catch (e) { updateSync('error', 'Error de connexió'); showToast('Error: ' + e.message, 'error'); }
}
...
function renderPlanning() {
  document.getElementById('planWeekTitle').textContent = getPlanWeekLabel(_planWeekOffset);
  ...
  PLAN_FRANGES.forEach(franja => {
    ...
    PLAN_DIES.forEach(dia => {
      ...
      html += renderPlanCell(data, key, isPati);
    });
    html += '</tr>';
  });
  html += '</tbody>';
...
function renderPlanCell(data, key, isPati) {
  if (!data) {
