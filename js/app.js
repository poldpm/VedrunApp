...
function showPage(pageId) {
  ...
  if (pageId === 'assoliments')  { renderAssoliments(); }
...
function _renderPIAMAssigs(tipus) {
  ...
  wrap.innerHTML = PIAM_ASSIGS.map(a =>
    `<button type="button" class="pi-am-assig-btn ${sel.includes(a)?'active':''}" onclick="_togglePIAMAssig('${tipus}','${a}')">${a}</button>`
  ).join('');
}
...
async function saveGrupsSheetCfg() {
  if (!config.scriptUrl) { showToast('Primer configura la connexió', 'error'); return; }
  const raw = document.getElementById('cfgGrupsSheet').value.trim();
  const id = _extractSheetId(raw);
  if (!id) { showToast('Enganxa l\'enllaç del full "grups"', 'error'); return; }
  const btn = document.getElementById('saveGrupsSheetBtn');
  if (btn) { btn.disabled = true; btn.textContent = 'Desant…'; }
  try {
    const r = await appsScriptPost({ action: 'saveGrupsSheetId', id: id });
    if (r.ok) showToast('Enllaç del full "grups" desat ✓', 'success');
    else showToast('Error: ' + (r.error || 'desconegut'), 'error');
  } catch(e) {
    showToast('Error desant l\'enllaç: ' + e.message, 'error');
  }
  if (btn) { btn.disabled = false; btn.textContent = 'Desar l\'enllaç'; }
}

async function saveDesdobSheetCfg() {
  if (!config.scriptUrl) { showToast('Primer configura la connexió', 'error'); return; }
  const raw = document.getElementById('cfgDesdobSheet').value.trim();
  const id = _extractSheetId(raw);
  if (!id) { showToast('Enganxa l\'enllaç del full de desdoblaments', 'error'); return; }
  const btn = document.getElementById('saveDesdobSheetBtn');
  if (btn) { btn.disabled = true; btn.textContent = 'Desant…'; }
  try {
    const r = await appsScriptPost({ action: 'saveDesdobSheetId', id: id });
    if (r.ok) showToast('Enllaç dels desdoblaments desat ✓', 'success');
    else showToast('Error: ' + (r.error || 'desconegut'), 'error');
  } catch(e) {
    showToast('Error desant l\'enllaç: ' + e.message, 'error');
  }
...
async function loadAll() {
  ...
  // 2) En segon pla, refresca amb les dades fresques del servidor (1 sola crida)
  try {
    const main = await appsScriptGet({ action: 'getMainData' });
    if (main.ok) {
      if (main.alumnes)  students = main.alumnes;
      if (main.registre) { registreItems = main.registre.items; registreData = main.registre.data; }
      if (main.observacions) observacions = main.observacions;
      if (main.personal) {
        personal = {};
        main.personal.forEach(function(p) {
    ...
    // Si el perfil té un grup de tutoria, carrega'n els alumnes del full centralitzat
    _loadTutoriaGrup();

    // === CÀRREGUES EN SEGON PLA (espaiades perquè no saturin Apps Script) ===
    // El botó ja és verd; aquestes no bloquegen ni afecten l'indicador.
    ...
    // Carrega el perfil del mestre (per mostrar el nom al menú)
    if (typeof _perfilLoadFromSheets === 'function') {
      _perfilLoadFromSheets().then(() => { if (typeof _perfilUpdateNav === 'function') _perfilUpdateNav(); });
    }
...
async function _backgroundRefresh() {
  ...
  _bgRefreshing = true;
  try {
    // Només 2 crides: dades principals consolidades + full de càlcul consolidat
    const main = await appsScriptGet({ action: 'getMainData' }, false);
    if (main && main.ok) {
      if (main.alumnes)  students = main.alumnes;
      if (main.registre) { registreItems = main.registre.items; registreData = main.registre.data; }
      if (main.observacions) observacions = main.observacions;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    ...
    if (pc) { _perfil = JSON.parse(pc); if (typeof _perfilUpdateNav === 'function') _perfilUpdateNav(); if (typeof perfilRenderNavAssigs === 'function') perfilRenderNavAssigs(); }
  } catch(e) {}
